---
title: "How well does a subset of a food web describe the entire food web?"
author: "Felix Zeh"
date: "2023-05-17"
...
### 2.5 Sampling from sub-food webs: random species
...
### 2.6 Sampling from sub-food webs: random species, but forced variation in trophic levels
...
I calculated pearson correlation coefficients between extinctions in the whole food web and the extinctions the two sub-foodwebs consisting of 24 species which have been sampled in chapter 2.4, using base R's `cor()` function. I similarly calculated pearson between Shannon-diversity in the whole food web and in the biggest/smallest consumers.

I then calculated pearson correlation coefficients for extinctions and Shannon-diversity between the entire food web and the 9 sub-foodwebs of varying size (4 to 36 species) which were sampled in chapters 2.5 and 2.6.

## 3 Results

Generally, overall number of extinctions increased with increasing connectance, as well as the variation in the number of extinctions (fig. 1a) . Shannon's diversity showed a strong decrease at a connectance between 0.05 and 0.20, but stayed relatively constant at connectances from 0.20 to 0.35 (fig. 1b).

```{r echo=FALSE}
library(ggplot2)

d <- read.csv("./data/20220506_96spec_2000c_04to12bas_v02.csv")[,-1]
